...
static
void bitfile_set (Bitfile * b, const void * start, size_t len)
{
  b->start = (const unsigned char *) start;
  b->seek  = 0;
  b->size  = len << 3;
}

/*
.. NOTE : TIFF use MSB ordering.
*/
...
unsigned int get_code (Bitfile * b, size_t len)
{
  ...
  const unsigned char * stream = b->start; 
  unsigned int code = 0;
...
int lzw_decode (const void * encoding, size_t len, void * buff, size_t outlen)
{
  ...
  while (1)
  {
    ...
    /* string corresponding to the code is appended to the output */ 
    while (depth--)
    {
      put (stack [depth]);
    }
